...
def classroom_login(local_driver):
    local_driver.get('https://accounts.google.com/o/oauth2/v2/auth/oauthchooseaccount?redirect_uri=https%3A%2F%2F'
...
async def librus_check_for_subjects(local_client):
    subjects = {}
    a = await local_client.fetch('/Subjects', syn_obj=SynSubject)
    for i in a:
        subjects[str(i)] = ''
    return subjects
...
def join_classroom(lesson):
    ...
    if classroom != '':
        local_driver = webdriver.Chrome(options=driver_options)
        local_driver.get('https://classroom.google.com')
        time.sleep(5)
        try:
            element = WebDriverWait(local_driver, 10).until(
        ...
        while loading:
            try:
                reload_button = WebDriverWait(local_driver, 1).until(
                    expected_conditions.presence_of_element_located(
